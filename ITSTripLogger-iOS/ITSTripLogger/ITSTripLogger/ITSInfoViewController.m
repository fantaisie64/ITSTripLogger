...
- (void)viewDidLoad
{
    ...
    if([iTSBeaconManager locationServiceEnabled]){
        [locationServiceSwitch setOn:YES];
    }else{
        [locationServiceSwitch setOn:NO];
    }
    //iTSBeaconManager = [ITSBeaconManager getInstance];
...
- (IBAction)locationServiceSwitchValueChanged:(id)sender {
    if([locationServiceSwitch isOn]){
        if(![iTSBeaconManager locationServiceEnabled]){
            [iTSBeaconManager setLocationServiceEnabled:YES];
            [iTSBeaconManager saveParameterPlist];
            //[locationServiceSwitch setOn:YES];
        }
    }else{
        if([iTSBeaconManager locationServiceEnabled]){
            [iTSBeaconManager setLocationServiceEnabled:NO];
            [iTSBeaconManager saveParameterPlist];
            //[locationServiceSwitch setOn:NO];
        }
    }
    //NSLog(@"location service:%@", [iTSBeaconManager locationServiceEnabled] ? @"YES" : @"NO");
}
@end
