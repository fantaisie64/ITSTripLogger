            ...
            if(result1.length==0){
                ...
                function (err, result2) {
                    ...
                    if(result2.length==0){
                        ...
                        //test
                        console.log("parked_time1 = " + parked_time1);
                        console.log("parked_time2 = " + parked_time2);
                        //1
                        if(parked_time1 < parked_time2){
                            console.log("1:parked_time1<parked_time2");
                        }else if(parked_time1 == parked_time2){
                            console.log("1:parked_time1==parked_time2");
                        }else if(parked_time1 > parked_time2){
                            console.log("1:parked_time1>parked_time2");
                        }
                        //2
                        if(parked_time1.getTime() < parked_time2.getTime()){
                            console.log("2:parked_time1<parked_time2");
                        }else if(parked_time1.getTime() == parked_time2.getTime()){
                            console.log("2:parked_time1==parked_time2");
                        }else if(parked_time1.getTime() > parked_time2.getTime()){
                            console.log("2:parked_time1>parked_time2");
                        }
                        //3
                        if(new Date(parked_time1).getTime() < new Date(parked_time2).getTime()){
                            console.log("3:parked_time1<parked_time2");
                        }else if(new Date(parked_time1).getTime() == new Date(parked_time2).getTime()){
                            console.log("3:parked_time1==parked_time2");
                        }else if(new Date(parked_time1).getTime() > new Date(parked_time2).getTime()){
                            console.log("3:parked_time1>parked_time2");
                        }
                        ...
                        if(lat1 != 0.0 && long1 != 0.0){
                            ...
                            if(d > 1.0){ //greater than 1 mile
                                connection.query('UPDATE cars SET parked_time = ?, parked_latitude = ?, parked_longitude = ? 
                                    WHERE bt_major = ? AND bt_minor = ?',
                                    [req.body.data.timestamp, req.body.data.latitude, req.body.data.longitude, req.body.data.bt_major, req.body.data.bt_minor], function (err, result2-1) {
                                    if (err){
                                ...
                                connection.query('UPDATE cars SET parked_time = ? WHERE bt_major = ? AND bt_minor = ?',
                                    [req.body.data.timestamp, req.body.data.bt_major, req.body.data.bt_minor], function (err, result2-2) {
                                    if (err){
                                        //res.json({mode:'insertCar', message: 'insertCarErrorFail-2-1'});
                                    }else{
    ...
    while(req.body.data[i] !== undefined){
        ...
        function (err, result2) {
            ...
            if(result2.length==0){
                ...
                var lat1 = result2[0].parked_latitude;
                var long1 = result2[0].parked_longitude;
                var parked_time2 = req.body.data[i].timestamp;
                var lat2 = req.body.data[i].latitude;
                var long2 = req.body.data[i].longitude;
                if(lat1 != 0.0 && long1 != 0.0){
                    //get distance
                    var R = 3969.6; // mile = 6371 km
                    var φ1 = lat1.toRadians();
                    ...
                    if(d > 1.0){ //greater than 1 mile
                        connection.query('UPDATE cars SET parked_time = ?, parked_latitude = ?, parked_longitude = ? 
                            WHERE bt_major = ? AND bt_minor = ?',
                            [req.body.data[i].timestamp, req.body.data[i].latitude, req.body.data[i].longitude, req.body.data[i].bt_major, req.body.data[i].bt_minor], function (err, result2-1) {
                            if (err){
                                //res.json({mode:'insertCar', message: 'insertCarErrorFail-2-1'});
                            }
                        });
                        ...
                        connection.query('UPDATE cars SET parked_time = ? WHERE bt_major = ? AND bt_minor = ?',
                            [req.body.data[i].timestamp, req.body.data[i].bt_major, req.body.data[i].bt_minor], function (err, result2-2) {
                            if (err){
                                //res.json({mode:'insertCar', message: 'insertCarErrorFail-2-1'});
                            }else{
                                req.body.data[i].latitude = lat1;
                                req.body.data[i].longitude = long1;
                            }
                        });
                    }
                }
        ...
        connection.query('INSERT INTO records SET ?', req.body.data[i], function (err, result3) {
            if (err){
